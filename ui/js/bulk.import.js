...
function handleSelectedFile(evt) {
  ...
  let employerID = ""
  if (evt.target.id != "bulk-job-uploader"){
    employerID = evt.target.id.substring("job-uploader-".length)
  }
...
var ExcelToJSON = function (employer) {
    ...
    reader.onload = function (e) {
      try {
        ...
        for (let index = 0; index < jobs.length; index++) {
          ...
          // Parsing due date and since toISOString() is late by 3 hours I have added 15 hours
          let dueDate = null
          if (jobs[index]["Due Date"] != null && jobs[index]["Due Date"] != undefined &&
            jobs[index]["Due Date"] != "") {

            dueDate = new Date(jobs[index]["Due Date"])
            dueDate.setHours(dueDate.getHours() + 15)
            dueDate = dueDate.toISOString()
          }

          let jobFormData = {
            "Title": jobs[index]["Title"], "Description": jobs[index]["Description"],
            "Employer": employer ? employer : jobs[index]["Employer"], "ContactType": jobs[index]["Contact Type"],
            "Link": jobs[index]["Link"], "ContactInfo": jobs[index]["Contact Info"],
            "Type": jobs[index]["Type"], "Sector": jobs[index]["Sector"],
            "Experience": jobs[index]["Experience"], "EducationLevel": jobs[index]["Education Level"],
            "Gender": jobs[index]["Gender"], "DueDate": dueDate, "PostType": jobs[index]["Post Type"]
          }
